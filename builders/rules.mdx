...
The floor starts at exactly the account size minus your reserve, so `R` is sized to cover the whole of the designed fall. It then rises as the trader banks gains: 30 % of every gain above account size locks into it, and it does not come back down until the trader claims a withdrawal, which resets it to account size. A worked example is on [Account Parameters and Fees](/builders/fees#the-floor-your-drawdown-buys).
